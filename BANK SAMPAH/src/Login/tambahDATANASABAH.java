 ...
 * @author HP
 */
public class tambahDATANASABAH extends javax.swing.JDialog {
    ...
    public tambahDATANASABAH(KELOLANASABAH parent, boolean modal) {
        super(parent, modal);
        initComponents();

        connect();

        textNASABAH.setText(String.valueOf(autoID()));
        textNASABAH.setEditable(false);
    ...
    private void jButtonSIMPANActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSIMPANActionPerformed
        ...
        try {
            // 1. Validasi Input (ID is auto, so we don't validate it for emptiness if it's read-only, but good to keep logic safe)
            if (!validasiInput()) {
                return;
            }
            ...
            // --- USE AUTO ID ---
            n.setIdNasabah(autoID()); // Always generate a fresh ID
            // -------------------

            n.setNamaNasabah(textNAMALENGKAP.getText().trim());
            n.setAlamatNasabah(textALAMAT.getText().trim());
            n.setTelepon(textTELEPON.getText().trim());
            n.setUsernameNasabah(username);
            n.setPasswordNasabah(textKATASANDI.getText().trim());
            n.setNamaIbu(textNAMAIBU.getText().trim());

            // Save Nasabah first
            em.persist(n);

            // Handle Saldo (if input)
            String saldoText = textSALDO.getText().trim();
            if (!saldoText.isEmpty()) {
                ...
                s.setIdNasabah(n); // Link to the new Nasabah

                em.persist(s);

                // Link back to Nasabah if needed (though mapping usually handles this via foreign key)
                n.setSaldo(s);
                em.merge(n);
            } else {
                // If no balance input, create 0 balance entry (Optional but recommended)
                Saldo s = new Saldo();
                s.setTotalSaldo(java.math.BigInteger.ZERO);
                s.setIdNasabah(n);
                em.persist(s);
                n.setSaldo(s);
                em.merge(n);
